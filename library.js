// on() イベント発火時の処理(イベント名と処理．同じイベント名に複数処理)
// fireEvent() ，イベント名に対応する関数をすべて実行
...
// GoogleSpreadSheetの設定シートのアクセスクラス
// get()
// set()     B列にset
// setNote() C列にset
...
// Slackに対する受信時処理(receiveMessage)，送信時処理(send)
loadSlack = function () {
...
loadGSTimesheets = function () {
  ...
  GSTimesheets.prototype._getSheet = function() {
    if(this._sheets["timesheet"]) return this._sheets["timesheet"];

    var sheet = this.spreadsheet.getSheetByName("timesheet");
    if(!sheet) {
      sheet = this.spreadsheet.insertSheet("timesheet");
      if(!sheet) {
...
//    rowNo += parseInt((date.getTime()-date.getTimezoneOffset()*60*1000)/(1000*24*60*60)) - parseInt((s.getTime()-s.getTimezoneOffset()*60*1000)/(1000*24*60*60));
  ...
  GSTimesheets.prototype.get = function(date) {
    Logger.log("date:%s", date);
    var sheet = this._getSheet("timesheet");
    ...
    return({ date: row[0], temp: row[1], humidity: row[2], illminance: row[3] });
  };

  GSTimesheets.prototype.set = function(date, params) {
    var row = this.get(date);
    _.extend(row, _.pick(params, 't', 'h', 'l'));
    
    var sheet = this._getSheet();
    var rowNo = this._getRowNo(date);
    var lastRow = sheet.getRange("A:A").getLastRow()+1;
    var data = [getNow(), params.t, params.h, params.l].map(function(v) {
      return v == null ? '' : v;
    });
    sheet.getRange("A"+lastRow+":"+String.fromCharCode(65 + this.scheme.columns.length - 1)+lastRow).setValues([data]);
